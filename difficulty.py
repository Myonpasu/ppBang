...
def diffs_iterative(graph, epsilon=1e-4, mean_diff=5):
    ...
    while delta >= epsilon:
        ...
        # new_diffs *= vertex_count * mean_diff / new_diffs.sum()
        delta = np.linalg.norm(new_diffs - diffs.a, ord=1) / (vertex_count * mean_diff)
        diffs.a = new_diffs
